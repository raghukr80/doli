...
/**
 *	\file       htdocs/core/modules/propale/doc/pdf_azurSGST.modules.php
 *	\ingroup    propale
 ...
 */
class pdf_azurSGST extends ModelePDFPropales
{
	var $db;
	var $name;
	var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "azurSGST";
		$this->description = $langs->trans('Quotation From MeeHaa Surgitech TN-SGST');
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='')
	{
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN))
		{
			...
			if (empty($hidetop))
			{
				$pdf->SetXY($this->posxlocaltax1-3, $tab_top+1);
