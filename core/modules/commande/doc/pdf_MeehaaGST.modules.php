...
/**
 *	\file       htdocs/core/modules/commande/doc/pdf_MeehaaGST.modules.php
 *	\ingroup    commande
 *	\brief      Fichier de la classe permettant de generer les commandes au modele MeehaaGST
 */
...
/**
 *	Classe to generate PDF orders with template MeehaaGST
 */
class pdf_MeehaaGST extends ModelePDFCommandes
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "MeehaaGST";
		$this->description = $langs->trans('Proforma Invoice from MeeHaa Surgitech TN-SGST');
		...
		$this->postotalht=174;
		if (! empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT)) $this->posxlocaltax2=$this->posxup;
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='')
	{
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN))
		{
			...
			if (empty($hidetop))
			{
				$pdf->SetXY($this->posxlocaltax2-2, $tab_top+1);
				$pdf->MultiCell($this->posxlocaltax1-$this->posxlocaltax2+1,2, $outputlangs->transnoentities("SGST"),'','C');
			}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $titlekey="Order")
	{
		...
		if ($this->emetteur->logo)
		{
			if (is_readable($logo))
			{
			    ...
			    $pdf->Image($logo, $this->marge_gauche, $posy, 0, 15);	// width=3 , max height=6
			}
			else
			{
				$pdf->SetTextColor(200,0,0);
		...
		if ($showaddress)
		{
			...
			$pdf->SetXY($posx,$posy+2);
                       		$pdf->SetFont('','B', $default_font_size);
			$pdf->MultiCell(80, 4, 'MeeHaa Surgitech ', 0, 'L');
                        	//	$pdf->SetFont('','', $default_font_size - 2);
                        	//	$pdf->MultiCell(82, 4, '(Consignee Agent of Shalex Overseas Pvt. Ltd.) ', 0, 'L');
			$pdf->SetFont('','', $default_font_size - 1);
                        		$pdf->MultiCell(84, 4, 'B204/2 Srinivasa complex ', 0, 'L');
			$pdf->MultiCell(86, 4, 'NGO B Colony ', 0, 'L');
                        		$pdf->MultiCell(88, 4, 'Tirunelveli-627007 ', 0, 'L');
                        		$pdf->MultiCell(90, 4, 'Tamil Nadu ', 0, 'L');
                        		$pdf->MultiCell(92, 4, 'Telephone:+91-80720 38810 ', 0, 'L');
                        		$pdf->SetFont('','B', $default_font_size - 1);
                        		$pdf->MultiCell(94, 4, 'GSTIN: 33ARWPR7068Q1ZH ', 0, 'L');
