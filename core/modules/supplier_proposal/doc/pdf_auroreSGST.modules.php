...
/**
 *	\file       htdocs/core/modules/propale/doc/pdf_auroreSGST.modules.php
 *	\ingroup    propale
 ...
 */
class pdf_auroreSGST extends ModelePDFSupplierProposal
{
	var $db;
	var $name;
	var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "auroreSGST";
		$this->description = $langs->trans('Supplier Proposal from MeeHaa Surgitech TN-SGST');
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='')
	{
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			if (empty($hidetop))
			{
				$pdf->SetXY($this->posxlocaltax1-5, $tab_top+1);
				$pdf->MultiCell($this->posxup-$this->posxlocaltax1+3,2, $outputlangs->transnoentities("CGST"),'','C');
			}
		}
